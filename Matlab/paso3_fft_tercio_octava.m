...
for i=1:length(fnom)
    ...
    xf2_promedio(i)= mean(xf2(finf_lazo:fsup_lazo));
...
for i=1:length(fnom)
    ...
    xf_promedio(i)=mean(xf(finf_lazo:fsup_lazo));
...
%     - A(find(ff>fnom(i)*0.98 & ff<fnom(i)*1.02,1) );% - xf_desvio(i);
% end
% figure;
% semilogx(fnom,xf2_desvio*0.1,fnom,(xf2_desvio-xf_desvio)*0.1,fnom,xf_desvio*0.1,freqnom,maxA*0.1,freqnom(3:33),-minA*0.1);
% grid ON;
% hold;
...
for i=1:length(fnom)
    xf2_desvio(i) = xf2_promedio(i)...
    - C(find(ff>fnom(i)*0.98 & ff<fnom(i)*1.02,1) );% - xf_desvio(i);
end
figure;
semilogx(fnom,xf2_desvio*0.1,fnom,(xf2_desvio-xf_desvio)*0.1,fnom,xf_desvio*0.1,freqnom,maxC*0.1,freqnom(3:33),-minC*0.1);
grid ON;
hold;
